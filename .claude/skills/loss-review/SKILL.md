...
4. **Hypothesize** — one sentence, falsifiable, about *why* the pattern
   ...
   something exotic:
   - assumed colony-wide memory (each ant's memory is private, persists only
     for its own lifetime, and dies with it — the board is the only shared state)
   - forgot the fog (acted on cells no friendly ant can see)
   ...
   - forgot the resolution order (merge → move → combat → feed — a doomed ant
     never eats first)
   - every ant chasing the same target (clumping, wasted turns)
