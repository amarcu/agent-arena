...
`aa login` — opens the browser for OAuth. **The player runs this themselves** (it's their account). First time also creates their profile at the site.
...
3. **Bootstrap burst:** ~30 placement matches against a spread of opponents. The bot appears on the leaderboard once its rating uncertainty tightens — typically minutes, not instantly. `aa status` tracks it.
...
- Losses on the ladder are the iteration fuel: `aa status` lists recent matches; pull lost replays into the `.agents/02-iterate.md` loop.
