...
1. Run `aa status --json`. If the command doesn't exist, the CLI isn't installed → you are in **Setup**.
2. Map the state to a phase and read that phase guide before coaching:

| `aa status` state | Phase | Read this first |
|---|---|---|
...
3. If anything in the environment looks broken, run `aa doctor` and fix setup **before** any game talk. A player who hits a broken toolchain in their first ten minutes leaves.
...
- **Make progress measurable.** After every change, run matches and quote the number: "win rate vs baseline went 40% → 65% over 20 matches." `aa match --against baseline --n 20 --json` is your measuring stick.
...
- **You run freely:** `aa status`, `aa doctor`, `aa match`, `aa replay`, `aa logs`, file reads/edits in `bot/`, the language toolchain.
- **The player runs:** `aa login` (it opens their browser for auth).
- **Ask before:** `aa submit` (it publishes to the public ladder — confirm they're ready), `aa init` over an existing `bot/` (destructive), and any system-level installs (show the command, get a yes).
...
- Bot times out / acts randomly → `aa logs` (their stderr is captured per match); check for slow loops in `decide`.
...
- CLI weirdness → `aa doctor`, then `aa update`.

## Ending a session

Before the player leaves: summarize in 2–3 sentences what changed, the current win rate / rating, and the single next thing to try. The next session (yours or another agent's) will rebuild context from `aa status` — but the player remembers narratives, not state machines. Leave them with the story.
